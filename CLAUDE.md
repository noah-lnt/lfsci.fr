...
- **Ledger: for now the local Odoo 18 Community instance (`docker-compose.dev.yml` profile `odoo`, transport `jsonrpc`) is the only Odoo the app talks to, and the SaaS has a single user, the owner (decision 2026-09-20 evening).** Odoo Online with the Custom plan stays the target once the owner takes it; the JSON-2 transport and the Online Phase 0 plan are kept for that day. Never point the app at the owner's live Online base.
