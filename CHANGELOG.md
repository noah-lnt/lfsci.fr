...
- Integrations: Odoo JSON-2 client with rate limit, typed operations, capability snapshot and fake server; S3 storage with local dev driver; Mistral OCR; Gladia; Resend outbound and inbound with Svix verification; smsmode with French mobile gating; INSEE, ADEME, IGN open data; Typst PDF templates.
...
- Worker: outbox dispatch and reconcile, document analysis, inbound email and SMS, voice transcription, monthly rent terms, nightly controls, deadlines, Odoo back-sync, PDF rendering, IRL refresh.
