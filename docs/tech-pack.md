...
| I11 | SMS | **smsmode**: outbound from €0.0312 HT, an **SDA virtual mobile number** for inbound replies | French sender-ID rules since 2026-03-01: alphanumeric IDs are one-way and generic words are refused, so a real long code is the sender | Twilio French long code (two-way confirmed, no pre-registration) | House rule from Windtransfer: validate `isValidNumber` and number type before any paid send |
...
| Closed periods | The refusal is surfaced as-is; the SaaS never shifts an accounting date to get past a lock (WF-12) |
...
- **Authorisation:** better-auth organization roles carry the coarse role (spec §3); a `permission` module in `packages/domain` computes the fine-grained decision from role × organization × legal entity × property × data nature (SEC-01) and is called by oRPC middleware, by the search layer before retrieval, and by download-link issuance. Unauthorised reads answer 404, never 403 (kycone lesson).
...
| D-10 | GitHub repository name and CI | `lfsci.fr` under the owner's account; GitHub Actions running `npm run check` and `npm run e2e` |
| D-11 | Embedding model, which pins the `embedding.vector` dimension | **Mistral `mistral-embed`** on the EU endpoint, 1024 dimensions and not configurable (verified 2026-09-19 on docs.mistral.ai); the schema carries `vector(1024)` accordingly. Another model is a column migration, or a second table if two models coexist |
