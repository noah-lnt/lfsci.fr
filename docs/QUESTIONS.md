...
1. **Odoo Online plan (D-01) — parked 2026-09-20 evening: the local Odoo 18 Community instance is the ledger for now, the SaaS serves the owner alone, and P1 to P4 of the plan wait until the Custom plan is taken.** Earlier the same day: The owner keeps Odoo Online for its bank reconciliation; self-hosted Community was considered and rejected. The external API is a Custom-plan feature: €37.40 per user per month billed monthly, €29.90 billed yearly on odoo.com/pricing-plan as of 2026-09-20. Remaining action: subscribe to Custom, then create the neutralised duplicate and the bot API key for Phase 0.
...
50. **Roles (SEC-01) — enforced 2026-09-20.** Every procedure now checks the member's role against one policy table: the owner-admin does everything; a delegated manager reads and writes everything except the partners' current accounts, approvals and the ops screen; an accountant reads everything including the current accounts and writes nothing; a partner reader sees the shared indicators and their own security settings only. Still yours: whether a partner reader must see their own current account (that needs a link from the membership to the partner's person, which the schema does not hold), whether the delegated manager may see tenants' full identities (the spec says no implicit access; today they can), and whether a refusal should answer 404 rather than 403.
