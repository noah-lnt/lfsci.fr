...
/**
 * RFC 4180: a quoted field may contain the delimiter, a newline and doubled
 * quotes, so the file is read character by character rather than split.
 */
...
export function parseCsv(text: string, options?: { delimiter?: string }): CsvTable {
  ...
  if (clean.trim() === "") {
    throw new AppError("VALIDATION", { message: "Le fichier est vide." });
  }
  const delimiter = options?.delimiter ?? sniffDelimiter(clean);
  const records = tokenize(clean, delimiter);
  const header = records[0];
  if (!header) {
    throw new AppError("VALIDATION", { message: "Le fichier ne contient aucune ligne." });
  }
  const headers = header.cells.map((cell) => cell.trim());
  if (headers.every((cell) => cell === "")) {
    throw new AppError("VALIDATION", {
      message: "La première ligne ne porte aucun nom de colonne.",
    });
  }
  return { delimiter, headers, records: records.slice(1) };
}

/** Header matching is name-based, so spacing, case and accents must not matter. */
...
/** The cells of one record, addressed by header name and never by position. */
